module.exports = {
  ...
  try {
   ...
   await strapi.entityService.create('api::bitacora.bitacora', {
    data: {
     ...
     Descripcion: 'Se ha creado un sector de desempeño',
     Accion: 'POST',
     Datos: data
    }
   });
 ...
 async beforeUpdate(e){
  try {
   const oldData = await strapi.service('api::sector-desempeno.sector-desempeno')
   .findOne(e.params.data.id);
   const newData = e.params.data;
   await strapi.entityService.create('api::bitacora.bitacora', {
    data: {
     admin_user: { id: e.params?.data?.updatedBy },
     Descripcion: 'Se ha modificado un sector de desempeño',
     Accion: 'PUT',
